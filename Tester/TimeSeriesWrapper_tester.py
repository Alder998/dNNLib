...
# 0. Data
data = pd.read_csv("C:\\Users\\alder\\Downloads\\energy_market_data_1p.xlsx")
data["Date"] = pd.to_datetime(data["Date"])
data["Date"] = data["Date"].dt.tz_localize(None)
if "Unnamed: 0" in data.columns:
    data.drop(columns=["Unnamed: 0"])
data = data[data["zone"]=="Italia Sardegna"].sort_values(by="Date", ascending=True).reset_index(drop=True)
...
# 1. Model
ts.TimeSeriesWrapper(modelStructure={"MultiSeasonConv1DGated": {"layers": [16, 16, 16, 16, 16], "cycles": [24, 48, 96, 192, 288],
