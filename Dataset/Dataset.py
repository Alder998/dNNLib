...
class Dataset:
    ...
    # Process Dataset for geo-space time series model
    def processDatasetForGeoSpaceTimeSeries (self, dataInDataFrameFormat, date_column, target_column, lag_series,
                                             space_variables, date_column_format="%Y-%m-%d", frequency="1h", classification=False):

        # The first part is the same as the one used for time-series model
        # 0.0. Check the compliance of the time-frequency
        if frequency not in ["1mo", "1d", "1h", "15min", "1min"]:
            raise Exception("Frequency not implemented! Available: '1mo', '1d', '15min', '1min'.")
        # 0.1. Convert to date column to datetime
        ...
        # 0.1. Sort by Date column
        dataInDataFrameFormat = dataInDataFrameFormat.sort_values(by=date_column, ascending=True).reset_index(
            drop=True)

        # 1. Add lags to the target variable, according specified by user
        dataInDataFrameFormat["unique_coord"] = dataInDataFrameFormat[space_variables].astype(str).agg("_".join, axis=1)
        if len(lag_series) != 0:
            for lag_number in lag_series:
                for tc in target_column:
                    for coord in dataInDataFrameFormat["unique_coord"].unique():
                        dataInDataFrameFormat.loc[dataInDataFrameFormat["unique_coord"]==coord, tc + "_" + str(lag_number) + "_lag"] = dataInDataFrameFormat[tc][dataInDataFrameFormat["unique_coord"]==coord].shift(lag_number)
        # 1.1. Eventually, drop the column used to isolate the coordinates
        dataInDataFrameFormat = dataInDataFrameFormat.drop(columns="unique_coord")
        # 2. Make the target variable categorical (only for classification problems)
        if classification:
            dataInDataFrameFormat, categories_map = self.processDatasetForClassification(
                dataInDataFrameFormat=dataInDataFrameFormat, target_column=target_column)
            return dataInDataFrameFormat.dropna().reset_index(drop=True), categories_map
